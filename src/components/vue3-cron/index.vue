        ...
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          ...
          <el-row>
            <el-radio v-model="state.second.cronEvery" label="4"
              ...
              <el-input-number size="small" v-model="state.second.rangeStart" :min="1" :max="60" />
              {{ state.text.Seconds.cycle[1] || '' }}
              <el-input-number size="small" v-model="state.second.rangeEnd" :min="0" :max="59" />
        ...
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          ...
          <el-row>
            <el-radio v-model="state.minute.cronEvery" label="4"
              ...
              <el-input-number size="small" v-model="state.minute.rangeStart" :min="1" :max="60" />
              {{ state.text.Minutes.cycle[1] }}
              <el-input-number size="small" v-model="state.minute.rangeEnd" :min="0" :max="59" />
        ...
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          ...
          <el-row>
            <el-radio v-model="state.hour.cronEvery" label="4"
              ...
              <el-input-number size="small" v-model="state.hour.rangeStart" :min="0" :max="23" />
              {{ state.text.Hours.cycle[1] }}
              <el-input-number size="small" v-model="state.hour.rangeEnd" :min="0" :max="23" />
        ...
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          ...
          <el-row>
            <el-radio v-model="state.day.cronEvery" label="2"
              ...
              <el-input-number size="small" v-model="state.day.incrementStart" :min="1" :max="31" />
              {{ state.text.Day.intervalDay[2] }}
            </el-radio>
          </el-row>
          <el-row>
            <el-radio class="long" v-model="state.day.cronEvery" label="3"
          ...
          <el-row>
            <el-radio v-model="state.day.cronEvery" label="4"
              >{{ state.text.Day.cycle[0] }}
              <el-input-number size="small" v-model="state.day.rangeStart" :min="0" :max="30" />
              {{ state.text.Day.cycle[1] }}
              <el-input-number size="small" v-model="state.day.rangeEnd" :min="0" :max="30" />
              {{ state.text.Day.cycle[2] }}
            </el-radio>
          </el-row>
          <el-row>
          ...
          <el-row>
            <el-radio class="long" v-model="state.day.cronEvery" label="6"
          ...
          <el-row>
            <el-radio v-model="state.day.cronEvery" label="12"
              ...
              <el-input-number size="small" v-model="state.week.cronNthDayNth" :min="1" :max="5" />
        ...
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          ...
          <el-row>
            <el-radio v-model="state.month.cronEvery" label="4"
              ...
              <el-input-number size="small" v-model="state.month.rangeStart" :min="1" :max="12" />
              {{ state.text.Month.cycle[1] }}
              <el-input-number size="small" v-model="state.month.rangeEnd" :min="1" :max="12" />
            </el-radio>
          </el-row>
        </div>
      </el-tab-pane>
        ...
        <div class="tabBody myScroller" :style="{ 'max-height': maxHeight }">
          ...
          <el-row>
            <el-radio v-model="state.year.cronEvery" label="4"
              ...
              <el-input-number size="small" v-model="state.year.rangeEnd" :min="2018" :max="2118" />
            </el-radio>
          </el-row>
        </div>
      </el-tab-pane>
...
import { reactive, computed, toRefs, defineComponent, watch } from 'vue'
  ...
  setup(props, { emit }) {
    ...
    const setValue = (value) => {
      const arr = (value || '* * * * * ? *')
        ...
        .filter((m) => m)
      // 先处理秒、分、时、月、年
      const items = ['second', 'minute', 'hour', 'day', 'month', 'week', 'year']
      ...
      if (arr.length === 7) {
        items.forEach((item, index) => {
          if (item != 'day' && item != 'week') {
            ...
            if (value === '*') {
              state[item].cronEvery = '1'
            } else if (value.indexOf('/') >= 0) {
              state[item].cronEvery = '2'
              const temp = value.split('/')
              state[item].incrementIncrement = parseInt(temp[0])
              state[item].incrementStart = parseInt(temp[1])
            } else if (/^[^-]+$/.test(value)) {
              state[item].cronEvery = '3'
              state[item].specificSpecific = value
                .split(',')
                .filter((m) => m !== '')
                .map((m) => parseInt(m))
            } else if (/^[\d]+-[\d]+$/.test(value)) {
              state[item].cronEvery = '4'
              const temp = value.split('-')
              state[item].rangeStart = parseInt(temp[0])
              state[item].rangeEnd = parseInt(temp[1])
            }
          }
        })
        ...
        if (day !== '?') {
          ...
          if (day === '*') {
            ...
            state.day.incrementIncrement = parseInt(temp[0])
            state.day.incrementStart = parseInt(temp[1])
          } else if (day === 'L') {
            state.day.cronEvery = '7'
          } else if (day === 'LW') {
            state.day.cronEvery = '8'
          } else if (day.endsWith('L')) {
            state.day.cronEvery = '9'
            state.day.cronLastSpecificDomDay = parseInt(day.replace('L', ''))
          } else if (day.startsWith('L-')) {
            state.day.cronEvery = '10'
            state.day.cronDaysBeforeEomMinus = parseInt(day.split('-')[1])
          } else if (/^[\d]+W$/.test(day)) {
            state.day.cronEvery = '11'
            state.day.cronDaysNearestWeekday = parseInt(day.replace('W', ''))
          } else if (/^[\d,]+[^L]$/.test(day)) {
            state.day.cronEvery = '3'
            state.day.specificSpecific = day
              .split(',')
              .filter((m) => m !== '')
              .map((m) => parseInt(m))
            ...
            const temp = value.split('-')
            state.day.rangeStart = parseInt(temp[0])
            state.day.rangeEnd = parseInt(temp[1])
          }
        } else if (week !== '?') {
          // 配置周
          if (week.indexOf('/') >= 0) {
            state.day.cronEvery = '5'
            const temp = week.split('/')
            state.week.incrementIncrement = parseInt(temp[0])
            state.week.incrementStart = parseInt(temp[1])
          } else if (week.indexOf('#') >= 0) {
            state.day.cronEvery = '12'
            const temp = week.split('#')
            state.week.cronNthDayDay = parseInt(temp[0])
            state.week.cronNthDayNth = parseInt(temp[1])
          } else if (/^[a-zA-Z,]+$/.test(week)) {
            state.day.cronEvery = '6'
            state.week.specificSpecific = week.split(',').filter((m) => m)
          }
        }
      }
    }

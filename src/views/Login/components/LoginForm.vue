...
import { loginApi, getTestRoleApi, getAdminRoleApi, getUser } from '@/api/login'
    ...
    if (isValid) {
      ...
      try {
        ...
        if (res && res.status == 0) {
          ...
          userStore.setUserInfo(user_data.data.dto)
          // 是否使用动态路由
          if (appStore.getDynamicRouter) {
            await getRoleRoutes(user_data.data.dto, (route) => addRoute(route))
